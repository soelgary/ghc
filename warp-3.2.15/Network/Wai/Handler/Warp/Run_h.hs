...
run p a = do
  runSettings defaultSettings { settingsPort = p } a
...
data Handle = Handle Settings (IO (Connection, Transport)) SockAddr Application Counter InternalInfo0

coreHandler :: MVar Handle -> [ThreadId] -> IO ()
coreHandler next_app tids = do
  timeRemaining <- getTime
  () <- print $ "Core handler running with " ++ (show timeRemaining) ++ " ticks"
  myTid <- myThreadId
  (cap,_) <- threadCapability myTid
  h <- takeMVar next_app
  if timeRemaining >= minTickSize-- * 2
    then enoughTime h cap
    else notEnoughTime h cap

  where
    enoughTime (Handle set mkConn addr app counter ii0) cap = do
      timeRemaining <- getTime
      let toRelease = 50--timeRemaining `div` 2
      () <- releaseTime toRelease
      tid <- fork set mkConn addr app counter ii0 cap toRelease defaulTimeout
      coreHandler next_app (tids ++ [tid])

    notEnoughTime (Handle set mkConn addr app counter ii0) cap = do
      let tid = head tids
      () <- forceOnQueue tid
      ticks <- hKillThread tid
      () <- print $ "Killed thread and reclaimed ticks " ++ (show ticks)
      tid <- fork set mkConn addr app counter ii0 cap ticks defaulTimeout
      let newTids = (tail tids) ++ [tid]
      coreHandler next_app newTids
...
acceptConnection set getConnMaker app counter ii0 = do
    ...
    let initialCapability = 1
    let timeout = 0
    mv1 <- newEmptyMVar
    mv2 <- newEmptyMVar
    mv3 <- newEmptyMVar
    t1 <- hForkOn 1 initialTickSize timeout (coreHandler mv1 [])
    t2 <- hForkOn 2 initialTickSize timeout (coreHandler mv2 [])
    t3 <- hForkOn 3 initialTickSize timeout (coreHandler mv3 [])
    let mvars = [mv1, mv2, mv3]
    let tids = [t1, t2, t3]
    void $ mask_ $ acceptLoop initialCapability mvars tids
    gracefulShutdown set counter
  where
    acceptLoop index mvars tids = do
        () <- print $ "start of accept loopski on cap " ++ (show index)
        -- Allow async exceptions before receiving the next connection maker.
        allowInterrupt
        () <- print $ "Done interrupting " ++ (show index)
        ...
        case mx of
            Nothing             -> do
              () <- print "accept loop done"
              return ()
            Just (mkConn, addr) -> do
              () <- print "Putting"
              let mv = mvars!!index
              let handle = Handle set mkConn addr app counter ii0
              --fork set mkConn addr app counter ii0
              --let handle = Handle set mkConn addr app counter ii0
              pid <- forkOn 0 (forward mv handle)
              --() <- putMVar mv handle
              numCaps <- getNumCapabilities
              let nextIndex = (index + 1) `mod` (numCaps - 1)
              acceptLoop nextIndex mvars tids

    forward mv handle = do
      () <- putMVar mv handle
      return ()
...
fork :: Settings
     ...
     -> Int
     -> IO ThreadId
fork set mkConn addr app counter ii0 core ticks timeout =
  settingsFork set core ticks timeout $ \unmask ->
  ...
  where
    ...
    closeConn ref conn = do
        ...
        () <- print $ "Closing connection progress " ++ (show isClosed)
